...
#define nwkcRouteDiscoveryTime          0x2710  // milliseconds
...
FinishBroadcastPacket:
        ...
        // Handle any pending route discovery maintenance
            ...
            if (nwkStatus.flags.bits.bAwaitingRouteDiscovery)
            {
                ...
                for (rdIndex = 0; rdIndex < ROUTE_DISCOVERY_TABLE_SIZE; rdIndex ++ )
                {
                    if (routeDiscoveryTablePointer[rdIndex] != NULL)
                    {
                        if ((routeDiscoveryTablePointer[rdIndex]->forwardRREQ) &&
                            ((currentTime.Val - routeDiscoveryTablePointer[rdIndex]->rebroadcastJitterTimer.Val) >= (DWORD)RREQ_BROADCAST_JITTER))
                        {
                ...
                {
                    if ((routeDiscoveryTablePointer[rdIndex] != NULL) &&
                        ((currentTime.Val - routeDiscoveryTablePointer[rdIndex]->timeStamp.Val) > (ONE_SECOND*nwkcRouteDiscoveryTime/1000)))
                    {
...
HandleRouteRequest:
                                {
                                    ...
                                    // Update the path cost to get to us.
                                        ...
                                        rssi = NWKGet();
                                        lqi = NWKGet();
                                        // TODO LQI needs to be properly scaled using RSSI
                                        if (lqi > 75)
                                            lqi = 1;
                                        else if (lqi > 50)
                                            lqi = 3;
                                        ...
                                        (params.MCPS_DATA_indication.DstAddr.ShortAddr.Val == 0xFFFF))
                                    {
                                        ...
                                        if (rdIndex < ROUTE_DISCOVERY_TABLE_SIZE)
                                        {
                                            ...
                                            if (rreq.pathCost <= routeDiscoveryTablePointer[rdIndex]->forwardCost)
                                            {
                                                ...
                                                if ((macPIB.macShortAddress.Val == rreq.destinationAddress.Val) ||
                                                    ...
                                                    (currentNeighborRecord.deviceInfo.bits.Relationship == NEIGHBOR_IS_CHILD)))
                                                {
                                                    // If the destination is one of my end devices,
                                                    // update the path cost to account for one more hop.
                                                    if (macPIB.macShortAddress.Val != rreq.destinationAddress.Val)
                                                    {
                                                        rreq.pathCost += 7;
                                                    }
                                        ...
                                        else // Route Discovery entry does not exist
                                        {
                                            ...
                                            if ((macPIB.macShortAddress.Val == rreq.destinationAddress.Val) ||
                                                ...
                                                (currentNeighborRecord.deviceInfo.bits.Relationship == NEIGHBOR_IS_CHILD)))
                                            {
                                                // If the destination is one of my end devices, update the path
                                                // cost to account for one more hop.
                                                if (macPIB.macShortAddress.Val != rreq.destinationAddress.Val)
                                                {
                                                    rreq.pathCost += 7;
                                                }
                                        ...
                                        if (forwardRouteRequest)    // && ZigBeeReady())
                                        {
                                            ...
                                            if ((routeDiscoveryTablePointer[rdIndex]->forwardRREQ = SRAMalloc(sizeof(ROUTE_REQUEST_COMMAND)+8)) != NULL)
                                            {
                                                BYTE *ptr2;
                                                ...
                                                *ptr2++ = nwkFrameControlLSB.Val;
                                                ...
                                                routeDiscoveryTablePointer[rdIndex]->rebroadcastJitterTimer = TickGet();
                                            }
                                ...
                                #endif
                                    {
                                                ...
                                                !IsDescendant( macPIB.macShortAddress, nwkSourceAddress, currentNeighborTableInfo.depth )))
                                        {
                                            // See if I am or one of my end devices is the destination of the route request.
                                            // Otherwise, unicast the route request.
                                            if ((NWKLookupNodeByShortAddrVal( rreq.destinationAddress.Val ) != INVALID_NEIGHBOR_KEY) &&
                                                (currentNeighborRecord.deviceInfo.bits.Relationship == NEIGHBOR_IS_CHILD))
                                            {
                                                // If the destination is one of my end devices, update the path
                                                // cost to account for one more hop.
...
#endif  // I_SUPPORT_ROUTING
                        {
                            ...
                            // Set up the packet for rebroadcast.  The packet must have a radius > 0 to retransmit
                            if (nwkRadius > 0)
                            {
                                if (CreateNewBTR( &BTTIndex ))
                                {
                                    ...
                                    nwkStatus.BTT[BTTIndex]->nwkRadius                    = nwkRadius;
...
#endif // I_AM_RFD
                    {
                            ...
                            if (nwkStatus.flags.bits.bCanRoute)
                            {
                                ...
                                else if (i == NWK_COMMAND_ROUTE_REPLY)
                                {
        ...
        #endif // USE_TREE_ROUTING_ONLY
                                    {
                                        ...
                                        {
                                            //rrep.pathCost += 7; TODO This is in the spec algorithm, but isn't this already in path cost from the destination???
                                            forwardRouteReply = TRUE;
                                        }
                                    }
                        ...
                        else
                        {
                            ...
                            else if (nwkStatus.flags.bits.bCanRoute && (nwkRadius != 0) && ZigBeeReady())
                            {
...
//                params.NLME_LEAVE_confirm.DeviceAddress.v[7] = MAC_LONG_ADDR_BYTE7;
            ...
            case MLME_BEACON_NOTIFY_indication:
                // If we did not get a short address, then the node cannot be a ZigBee node.
                if (params.MLME_BEACON_NOTIFY_indication.CoordAddrMode == 0x02)
                {
                    ...
                    {
                        // This is a new node, so try to add it to the neighbor table.
                        if (CanAddNeighborNode() != INVALID_NEIGHBOR_KEY)
                        {
...
//                }
            ...
            case MLME_RESET_confirm:
                // Reset all NWK layer variables and tables.
...
StartActiveScan:
                    ...
                    else // active scan
                    {
                        ...
                        // Clear the neighbor table to remove nodes on other networks.
                        NWKClearNeighborTable();
                    ...
                    else
                    {
                        ...
                        if (!params.NLME_JOIN_request.RejoinNetwork)
                        {
                            ...
                            if (currentNeighborTableInfo.parentNeighborTableIndex != MAX_NEIGHBORS)
                            {
                                ...
                                params.MLME_ASSOCIATE_request.CapabilityInformation.Val = MY_CAPABILITY_INFO;
...
void CreateRouteReply( SHORT_ADDR nwkSourceAddress, BYTE rdIndex, ROUTE_REQUEST_COMMAND *rreq )
{
    ...
    TxBuffer[TxHeader--] = rreq->destinationAddress.byte.MSB;           // source address
    TxBuffer[TxHeader--] = rreq->destinationAddress.byte.LSB;
...
MESSAGE_ROUTING_STATUS InitiateRouteDiscovery( BOOL fromUpperLayers, BYTE discoverRoute )
{
    ...
    else
    {
        ...
        // Buffer the current message.  Create the NWK header first.
        *ptr   = NWKGet();      // frameCONLSB.Val;
        *ptr++ = (*ptr & 0x3F) | discoverRoute;  // Put in new route discovery field
    ...
    // Prepare the Route Request message. Reset the pointers for the new message.
    TxData = TX_DATA_START;
    TxHeader = TX_HEADER_START;

    // Load up the NWK payload - the route request command frame.
    TxBuffer[TxData++] = NWK_COMMAND_ROUTE_REQUEST;
    TxBuffer[TxData++] = ROUTE_REPAIR;                  // Route options; we will always use Route Repair
    TxBuffer[TxData++] = nwkStatus.routeRequestID++;
    TxBuffer[TxData++] = nwkStatus.routingMessages[messageIndex]->destinationAddress.byte.LSB;
    TxBuffer[TxData++] = nwkStatus.routingMessages[messageIndex]->destinationAddress.byte.MSB;
    ...
    TxBuffer[TxHeader--] = macPIB.macShortAddress.byte.MSB;
    TxBuffer[TxHeader--] = macPIB.macShortAddress.byte.LSB;
    TxBuffer[TxHeader--] = nwkStatus.routingMessages[messageIndex]->destinationAddress.byte.MSB;
    TxBuffer[TxHeader--] = nwkStatus.routingMessages[messageIndex]->destinationAddress.byte.LSB;
    TxBuffer[TxHeader--] = 0; // nwkFrameControlMSB TODO check security setting for this
    TxBuffer[TxHeader--] = NWK_FRAME_CMD | (nwkProtocolVersion<<2);    // nwkFrameControlLSB
...
void MarkNeighborAsPasssiveACKed( BYTE BTTindex )
{
    ...
    while ((j < MAX_NEIGHBORS) && (currentNeighborRecord.shortAddr.Val != params.MCPS_DATA_indication.SrcAddr.ShortAddr.Val));
...
void SetBeaconPayload( void )
{
    macPIB.macBeaconPayload[0] = 0;    // Protocol ID, 0 by definition
