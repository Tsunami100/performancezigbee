...
//                        (TickGetDiff(TickGet(), pFirstEndBindRequest->lastTick) >= CONFIG_ENDDEV_BIND_TIMEOUT) )
                    {
                        pFirstEndBindRequest->flags.bits.fResponse = END_DEVICE_BIND_TIMEOUT;
...
Finished_NWK_ADDR_req:
                        ...
                        case SIMPLE_DESC_req:
                            ...
                            if ( IsThisMyShortAddr())
                            {
                                ...
                                {
SendInvalidEndpoint:                // Invalid Endpoint
            ...
            case ZDO_END_DEVICE_BIND_req:
                {
                    ...
                    if ((bindInfo = (END_DEVICE_BIND_INFO *)SRAMalloc( sizeof(END_DEVICE_BIND_INFO) )) != NULL)
                    {
                        ...
                        bindInfo->EP                 = params.ZDO_END_DEVICE_BIND_req.endpoint;
...
//        TxBuffer[TxData++] = MAC_LONG_ADDR_BYTE1;
//        TxBuffer[TxData++] = MAC_LONG_ADDR_BYTE2;
//        TxBuffer[TxData++] = MAC_LONG_ADDR_BYTE3;
//        TxBuffer[TxData++] = MAC_LONG_ADDR_BYTE4;
//        TxBuffer[TxData++] = MAC_LONG_ADDR_BYTE5;
//        TxBuffer[TxData++] = MAC_LONG_ADDR_BYTE6;
//        TxBuffer[TxData++] = MAC_LONG_ADDR_BYTE7;
...
//                      TxData += sizeof(currentNeighborRecord.longAddr;
            ...
            PrepareMessageResponse( clusterID ); // Short Addresses
//          PrepareMessageResponse( clusterID ); // Long Addresses
        #endif
    }
}
...
END_DEVICE_BIND_INFO * GetOneEndDeviceBindRequest( END_DEVICE_BIND_INFO * pRequestInfo )
{
    ...
    if (pRequestInfo == NULL)
    {
        ...
        if ((pEndDeviceBindRequest = (END_DEVICE_BIND_INFO *)SRAMalloc( sizeof(END_DEVICE_BIND_INFO) )) != NULL)
        {
            ...
            pEndDeviceBindRequest->EP                    = ZDOGet();
...
ZIGBEE_PRIMITIVE ProcessEndDeviceBind( END_DEVICE_BIND_INFO *pRequestInfo )
{
    ...
    status = END_DEVICE_BIND_NO_MATCH;
    nextPrimitive = NO_PRIMITIVE;
                 ...
                 firstIndex++, firstClusterPtr++)
            {
                     ...
                     secondIndex++, secondClusterPtr++)
                {
                    if (*firstClusterPtr == *secondClusterPtr)
                    {
                        ...
                        {
                            if (APSRemoveBindingInfo(pSecondEndBindRequest->shortAddr, pSecondEndBindRequest->EP,
                                    *secondClusterPtr, pFirstEndBindRequest->shortAddr, pFirstEndBindRequest->EP) == SUCCESS)
                        ...
                        {
                            APSAddBindingInfo(pSecondEndBindRequest->shortAddr, pSecondEndBindRequest->EP,
                                    *secondClusterPtr, pFirstEndBindRequest->shortAddr, pFirstEndBindRequest->EP);
                        }
                        else
                        {
                            APSRemoveBindingInfo(pSecondEndBindRequest->shortAddr, pSecondEndBindRequest->EP,
                                    *secondClusterPtr, pFirstEndBindRequest->shortAddr, pFirstEndBindRequest->EP);
                        }
                    }
                }
                 ...
                 firstIndex++, firstClusterPtr++)
            {
                     ...
                     secondIndex++, secondClusterPtr++)
                {
                    if (*secondClusterPtr == *firstClusterPtr)
                    {
                        ...
                        if (bindDirection == BIND_NOT_DETERMINED)
                        {
                            if (APSRemoveBindingInfo(pFirstEndBindRequest->shortAddr, pFirstEndBindRequest->EP,
                                    *firstClusterPtr, pSecondEndBindRequest->shortAddr, pSecondEndBindRequest->EP) == SUCCESS)
                                bindDirection = UNBIND_NODES;
                            else
                                bindDirection = BIND_NODES;
                        }
                        if (bindDirection == BIND_NODES)
                        {
                            APSAddBindingInfo(pFirstEndBindRequest->shortAddr, pFirstEndBindRequest->EP,
                                    *firstClusterPtr, pSecondEndBindRequest->shortAddr, pSecondEndBindRequest->EP);
                        }
                        else
                        {
                            APSRemoveBindingInfo(pFirstEndBindRequest->shortAddr, pFirstEndBindRequest->EP,
                                    *firstClusterPtr, pSecondEndBindRequest->shortAddr, pSecondEndBindRequest->EP);
                        }
                    }
                }
            ...
            if (CurrentRxPacket == NULL)
            {
                if ((CurrentRxPacket = SRAMalloc(4)) != NULL)
                {
                    ...
                    *ptr = status;
                    ...
                    params.APSDE_DATA_indication.ProfileId.Val          = MY_PROFILE_ID;
...
void PrepareMessageResponse( BYTE clusterID )
{
    ...
    TxBuffer[TX_DATA_START+1]   = APLGetTransId();                      // Transaction Sequence Number
    ...
    params.APSDE_DATA_request.SrcEndpoint   = 0x00;
    params.APSDE_DATA_request.DstEndpoint   = 0x00;
...
BOOL ProcessBindAndUnbind( BYTE bindInfo, LONG_ADDR *sourceAddress, LONG_ADDR *destinationAddress )
{
    ...
    if (params.ZDO_BIND_req.Status == SUCCESS)
    {
        ...
        if ((bindInfo & BIND_DIRECTION_MASK) == BIND_NODES)
        {
            ...
            {
                params.ZDO_BIND_req.Status = BIND_TABLE_FULL;
            }
        }
        else
        {
